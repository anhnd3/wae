...
<title>WAE Tools - Project Listing</title>
				...
				<div class="col-lg-12">
					<div class="panel panel-default">
						<div class="panel-heading">
							Project listing <a href="project-form"><button type="button"
							...
							<table style="width: 100%"
								...
								<tbody>
									<c:forEach items="${projects }" var="tmpProject">
										<tr>
											<td>${tmpProject.id }</td>
											<td>${tmpProject.name }</td>
											<td><img alt="228x114" src="${tmpProject.thumbnail }"
												style="width: 100px; height: 50px;"></td>
											<td>${tmpProject.author }</td>
											<c:if test="${tmpProject.highlight == true }">
												<td><i class="fa fa-check"></i></td>
											</c:if>
											<c:if test="${tmpProject.highlight == false }">
												<td><i class="fa fa-ban"></i></td>
											</c:if>
											<c:if test="${tmpProject.status == true }">
												<td><i class="fa fa-check"></i></td>
											</c:if>
											<c:if test="${tmpProject.status == false }">
												<td><i class="fa fa-ban"></i></td>
											</c:if>
											<td><a href="project-detail?id=${tmpProject.id }"><i
													class="fa fa-edit"></i> </a>| <a
												onclick="return confirm('Do you want to delete project: ${tmpProject.name}?')"
												href="project-delete?id=${tmpProject.id }"><i
